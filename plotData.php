...
//    echo $startTime."\n";
    $query = "SELECT time,temp,tcTemp,tcSwitch,thTemp,thSwitch FROM actionGraph WHERE id='".$_POST["graphId"]."' AND time>='".$startTime."' AND netID='".$netID."'";
...
*/
    $query = "SELECT time,temp,tcTemp,tcSwitch,thTemp,thSwitch FROM actionGraph WHERE id='".$_POST["graphId"]."' AND time>='".$startDate->format("U")."' AND time<='".$endDate->format("U")."' AND netID='".$netID."'";
//    echo $query."\n";
    $timeSpan = $endDate->format("U") - $startDate->format("U");
//    echo "timeSpan = ".$timeSpan."\n";
  }else{
    $query = "SELECT time,temp,tcTemp,tcSwitch,thTemp,thSwitch FROM actionGraph WHERE id='".$_POST["graphId"]."' AND netid='".$netID."'";
  }
//  echo $query."\n";
...
input:focus, textarea:focus{background-color: lightgrey;}
  ...
  <body>
    <?php 
      include ("header.html");
      $query = "SELECT time FROM actionGraph WHERE id='".$_POST["graphId"]."' AND netID='".$netID."'";
      $result = mysqli_query($link, $query);
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="1">
      ...
      <tr>
        <td valign="top" align="center" colspan="2" width="100%">
          <h2><?php echo $netName." Action #".$_POST["graphId"]; ?> Graph</h2>
        </td>
      </tr>
      <tr>
        <td align="center">
          <form method="post" action="plotData.php">
            <?php
              $formStr =
                "<input type=\"hidden\" name=\"graphId\" value=\"".$_POST["graphId"]."\">
